    ...
    while True:
        with devices_lock:
            ...
            for device_id, device_data in devices.items():
                if device_data.get('status') == 'ONLINE' and 'last_heartbeat' in device_data:
                    if isinstance(device_data['last_heartbeat'], datetime):
                        if (now - device_data['last_heartbeat']).total_seconds() > 15:
...
def handle_connect():
    ...
    with devices_lock:
        socketio.emit('devices_update', prepare_devices_for_emit(devices), room=request.sid)
